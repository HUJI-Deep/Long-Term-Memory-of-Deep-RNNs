...
m = args.m
assert m <= args.T, 'm must be smaller than T//2'
assert m >= 2, 'the minimal valid m is 2'
alphabet = np.array([chr(x) for x in range(A_ASCII, A_ASCII + n)] + (['-'] if (m < args.T//2) else []))
char_to_i = {c: i for (i, c) in enumerate(alphabet)}
conf_name = get_conf_name(C, n, m, args.rnn_cell, args.rnn_depth, args.rnn_hidden_dim,
...
def train_and_evaluate():
    # set utils globals
    set_globals(n, C, args, m, alphabet, char_to_i)
    # get data
